...
func TestSignupUser(t *testing.T) {
	...
	tests := []struct {
		...
		{"Short password", "Bob", "bob@example.com", "pa$$", csrfToken, http.StatusOK, []byte("This field is too short (minimum is 5 characters)")},
...
func TestCreateSnippetForm(t *testing.T) {
	...
	tests := []struct {
		...
		{"Unauthenticated", false, "","", http.StatusSeeOther, "Location", []byte("/user/login"), nil},
		{"Authenticated", true, "alice@example.com", "", http.StatusOK, "", nil, []byte("<form action='/snippet/create' method='POST'>")},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if !tt.auth && !bytes.Contains([]byte(header.Get(tt.wantHeader)), tt.headerContain) {
				t.Errorf("want header to contain %q", tt.headerContain)
			}
			if tt.auth && !bytes.Contains(body, tt.wantBody) {
				t.Errorf("want body %s to contain %q", body, tt.wantBody)
			}
		})
	}
